    ...
    <>
      ...
      <div className="px-4 pb-4">
        {(() => {
          ...
          const s = scheduleData[dayType];
          const n = s.length;
          let autoIdx = 0;
          if (curMins >= toMins(s[s.length - 1].time)) { autoIdx = s.length - 1; }
          else { for (let i = 0; i < s.length - 1; i++) { if (curMins >= toMins(s[i].time) && curMins < toMins(s[i + 1].time)) { autoIdx = i; break; } } }
          const item = s[autoIdx];
                  ...
                  {s.map((ev, idx) => {
                    ...
                    return (
                      <button key={idx} onClick={() => tapDetail && setScheduleModal({ title: ev.title, subtitle: ev.subtitle, detail: tapDetail, color: ev.color })} className="flex-1 flex flex-col items-center gap-2 active:opacity-60 transition-opacity">
                        <div style={{ marginTop: isCurrent ? 11 : 14, width: isCurrent ? 15 : 8, height: isCurrent ? 15 : 8, borderRadius: "50%", background: isCurrent ? ev.color : isPast ? "#cdd0d1" : "white", border: `2px solid ${isCurrent ? ev.color : isPast ? "#cdd0d1" : "#dde0e1"}`, boxShadow: isCurrent ? `0 0 0 4px ${ev.color}1e` : "none", flexShrink: 0, transition: "all 0.25s" }} />
                        <span className="leading-none transition-all" style={{ fontSize: isCurrent ? 10 : 8.5, fontWeight: isCurrent ? 700 : 500, color: isCurrent ? ev.color : isPast ? "#bbbec0" : "#d0d3d4" }}>{fmtTime(ev.time)}</span>
                      </button>
                    );
                  })}
                </div>
