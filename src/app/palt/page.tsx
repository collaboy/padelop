          ...
          <div style={{ width: "33.333%", flexShrink: 0, height: "100%", paddingLeft: 10, paddingRight: 10, position: "relative", zIndex: 2, overflow: "hidden" }}>
            <div style={{
              ...
              {(() => {
                ...
                const isSleepytime = now.getHours() < 7 || (isDone && !nextSlide);
                ...
                if (isDone) {
                  ...
                  if (nextSlide) {
                    ...
                    const [nh, nm] = nextSlide.time.split(":").map(Number);
                    const nextMs = new Date(now.getFullYear(), now.getMonth(), now.getDate(), nh, nm, 0).getTime();
                    if (doneAt && nextMs > doneAt) {
                      p = Math.min(1, Math.max(0, (Date.now() - doneAt) / (nextMs - doneAt)));
                    }
                  }
