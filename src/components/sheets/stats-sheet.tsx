...
function PadlaPtsRow({ points, expanded, onToggle }: { points: number; expanded: boolean; onToggle: () => void }) {
  ...
  return (
    <ExpandableRow color="#1a1c1c" title="Padla Pts" value={points > 0 ? points : "—"} sub="completed today" expanded={expanded} onToggle={onToggle}>
      ...
      <p style={{ fontSize: 11, fontWeight: 700, letterSpacing: "0.07em", textTransform: "uppercase", color: "#8a9096", margin: "0 0 10px" }}>Activity breakdown</p>
      {entries.length === 0 ? (
        <p style={{ fontSize: 14, color: "#9aa0a6", margin: 0 }}>No activities yet. Start completing tasks on the home screen.</p>
      ) : (
        <div style={{ display: "flex", flexDirection: "column", gap: 10 }}>
          {visible.map(([title, count]) => (
            <div key={title} style={{ display: "flex", alignItems: "center", justifyContent: "space-between" }}>
              <span style={{ fontSize: 14, fontWeight: 500, color: "#1a1c1c" }}>{title}</span>
