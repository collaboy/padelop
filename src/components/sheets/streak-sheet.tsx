  ...
  return (
    <div style={{ display: "flex", flexDirection: "column", gap: 14 }}>
      <div style={{ background: `linear-gradient(145deg, ${stier.grad[0]}, ${stier.grad[1]})`, borderRadius: 14, padding: "16px 20px", textAlign: "center" }}>
        <span style={{ fontSize: 15, fontWeight: 700, letterSpacing: "0.12em", textTransform: "uppercase", color: stier.color }}>{stier.label}</span>
        <p style={{ margin: "6px 0 3px", fontSize: 56, fontWeight: 800, color: stier.color, lineHeight: 1 }}>{streak}</p>
        <p style={{ margin: 0, fontSize: 21, fontWeight: 600, color: stier.color, opacity: 0.7 }}>day streak</p>
        <p style={{ margin: "10px 0 0", fontSize: 21, fontWeight: 500, color: "#4b5563" }}>{msg}</p>
      </div>
        ...
        {STIERS.map(t => {
          ...
          return (
            <div key={t.label} style={{ flex: 1, display: "flex", flexDirection: "column", alignItems: "center", gap: 4 }}>
              ...
              <span style={{ fontSize: 13, fontWeight: active ? 800 : 600, color: active ? t.color : "#9aa0a6", textAlign: "center", lineHeight: 1.2, letterSpacing: "0.04em" }}>{t.label}</span>
              <span style={{ fontSize: 13, color: "#b0b8c1", fontWeight: 500 }}>{t.min === 0 ? "0" : `${t.min}d`}</span>
            </div>
          );
        })}
      </div>
