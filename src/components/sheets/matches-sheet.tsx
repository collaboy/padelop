...
export default function MatchesSheet({ open, onClose }: Props) {
  ...
  if (!open && !selectedReview) return null;
                ...
                {matchHistoryOpen && (
                  <div style={{ display: "flex", flexDirection: "column", gap: 1, paddingBottom: 8 }}>
                    {[...reviews].sort((a, b) => b.ts.localeCompare(a.ts)).map((r, i) => {
                      ...
                      return (
                        <button key={i} onClick={() => { onClose(); setSelectedReview(r); }} style={{ width: "calc(100% - 16px)", alignSelf: "center", background: "#fff", borderRadius: 12, padding: "12px 14px", display: "flex", alignItems: "center", gap: 12, border: "none", cursor: "pointer", textAlign: "left" }}>
