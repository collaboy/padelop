...
function Avatar({ name, color, size = "md" }: { name: string; color: string; size?: "md" | "lg" }) {
  ...
  return (
    <div
      className={`${dim} rounded-full flex items-center justify-center font-black text-white ring-2 ring-white flex-shrink-0`}
...
function StatusPill({ status }: { status: "confirmed" | "pending" }) {
  return (
    <span
      className="flex-shrink-0 text-[11px] font-bold tracking-wide px-3 py-1.5 rounded-full uppercase"
      style={{
        background: status === "confirmed" ? "#caecbc" : "#fef3c7",
        color: status === "confirmed" ? "#496640" : "#92400e",
      }}
    >
  ...
  return (
    <div className="bg-white rounded-[24px] border border-[#e2e2e2] overflow-hidden" style={card}>
      ...
      <div className="px-5 pt-5 pb-4 flex justify-between items-start gap-3">
        <div>
          <p className="text-[15px] font-bold text-[#496640] leading-snug uppercase tracking-wide">
            {match.matchType ?? "Friendly Match"}
          </p>
      ...
      <div className="flex items-center justify-between px-5 py-4">
        ...
        <button className="text-[14px] font-bold text-[#1a1c1c] active:opacity-50 transition-opacity">
          Details
        </button>
      </div>
    </div>
  ...
  return (
    <div className="bg-white rounded-[20px] border border-[#e2e2e2] overflow-hidden" style={card}>
      <div className="px-5 pt-4 pb-3 flex justify-between items-start gap-3">
        <div>
          <p className="text-[13px] font-bold text-[#496640] uppercase tracking-wide">
            {match.matchType ?? "Friendly Match"}
          </p>
...
const NEXT_MATCH: MatchEntry = {
  ...
  status: "confirmed",
  partner: "Alex Ramos",
  opponent1: "Carlos Vega",
  opponent2: "David Puig",
  club: "Club Padel BCN",
};
...
export default function MatchesClient() {
  ...
  return (
    <div className="bg-[#f9f9f9] min-h-screen pb-28" style={{ fontFamily: "'Inter', sans-serif" }}>
      {/* Header */}
      <div className="pt-[88px] px-5 pb-5 flex items-start justify-between gap-3">
        <div>
          <h2 className="text-[22px] font-bold tracking-tight text-[#1a1c1c]">My Matches</h2>
          <p className="text-[14px] text-[#747878] mt-0.5"><span className="font-semibold text-[#1a1c1c]">Interesting...</span> you win 73% of matches when you take the first set.</p>
        </div>
        <button
          onClick={() => setFilterOpen(true)}
          className="mt-1 flex-shrink-0 w-9 h-9 rounded-full bg-white border border-[#e2e2e2] flex items-center justify-center active:scale-90 transition-transform relative"
          style={card}
        >
          ...
          {tab !== "All" && (
            <span className="absolute -top-0.5 -right-0.5 w-2 h-2 rounded-full bg-[#496640]" />
          )}
        </button>
      </div>
        ...
        <div className="fixed inset-0 z-50 flex items-center justify-center px-6" onClick={() => setFilterOpen(false)}>
          ...
          <div className="relative bg-white rounded-[24px] w-full max-w-xs overflow-hidden" style={card} onClick={e => e.stopPropagation()}>
            ...
            {TABS.map((t, i) => (
              <button
                key={t}
                onClick={() => { setTab(t); setFilterOpen(false); }}
      ...
      <div className="px-5 space-y-3">
        {showUpcoming && (
          <>
            <p className="text-[11px] font-semibold text-[#747878] uppercase tracking-widest pt-1">Next Match</p>
            <NextMatchCard match={NEXT_MATCH} />
            <p className="text-[11px] font-semibold text-[#747878] uppercase tracking-widest pt-2">Upcoming</p>
        ...
        {showPast && (
          <>
            <p className="text-[11px] font-semibold text-[#747878] uppercase tracking-widest pt-2">Past</p>
            <div className="space-y-3">
