                ...
                <div ref={tileRowRef} onScroll={e => setTileScrolled((e.currentTarget.scrollLeft) > 30)} style={{ overflowX: "scroll", scrollbarWidth: "none", msOverflowStyle: "none" } as React.CSSProperties}>
                  <div style={{ display: "flex", gap: 10 }}>
                    {([
                      ...
                      { label: "My Game", action: () => { if (pathname.startsWith("/my-game")) { setLogPickerOpen(false); return; } startNavLoad(); router.push("/my-game"); }, icon: <svg width="22" height="22" viewBox="0 0 24 24" fill="none" stroke="#6b7480" strokeWidth="1.8" strokeLinecap="round" strokeLinejoin="round"><ellipse cx="12" cy="8.5" rx="6.5" ry="7.5"/><path d="M10 15.5 L10.5 21 Q12 23 13.5 21 L14 15.5"/><line x1="9.8" y1="19" x2="14.2" y2="19"/><circle cx="12" cy="7" r="0.9" fill="#6b7480" stroke="none"/><circle cx="9.8" cy="9.5" r="0.9" fill="#6b7480" stroke="none"/><circle cx="14.2" cy="9.5" r="0.9" fill="#6b7480" stroke="none"/><circle cx="12" cy="12" r="0.9" fill="#6b7480" stroke="none"/></svg> },
                      ...
                      <button key={label} onClick={action} className="active:scale-95 transition-transform"
                        style={{ flex: "0 0 calc((100vw - 52px) / 3)", background: "none", border: "none", padding: "0 0 8px", cursor: "pointer", display: "flex", flexDirection: "column", alignItems: "center", gap: 5 }}>
                        <div style={{ width: "100%", aspectRatio: "1/1", borderRadius: "50%", background: active ? "#e0e2e6" : "#f0f1f4", display: "flex", alignItems: "center", justifyContent: "center" }}>
                          {icon}
                        </div>
                        <span style={{ fontSize: 11, fontWeight: 500, color: "#9aa5b0" }}>{label}</span>
                      </button>
                    ))}
                  </div>
                </div>
